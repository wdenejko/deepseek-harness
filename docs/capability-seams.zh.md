...
flowchart LR
  ...
  svc_localModels["ctx.localModels<br/>Remote local-model lifecycle owner"]
  pkg_web["web"]
  ...
  pkg_local_models --> svc_localModels
  pkg_lsp --> svc_lsp
...
| `ctx.localModels` | `core` | [`local-models`](../packages/local-models/local-models) | - | - | - | 可选启用、面向操作者的服务，不在随附默认组合中：通过 ctx.subprocess（ssh）发现远程主机上的 run 脚本，探测模型端点以获取运行状态，并一次启动/停止一个服务器，从而让模型下拉框能够列出、显示并切换正在运行的本地模型。apiproxy 的 localModels.* RPC 域通过 ctx.get 消费它。 |
