...
The shipped default search provider is now SearXNG, not `deepseek-official` — see [SearXNG default search](2026-08-22-searxng-default-search.md). This note's rationale for shipping `web_search` as a cross-surface default, the fetch-disabled stance, and the DeepSeek provider's credential-resolution mechanism remain current; the DeepSeek search row is now opt-in rather than the shipped default.
